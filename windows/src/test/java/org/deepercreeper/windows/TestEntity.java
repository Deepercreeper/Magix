...
public abstract class TestEntity extends Entity
{
    ...
    public Image generateImage(double scale)
    {
        ...
        Rectangle overlayRectangle = new BoxBuilder().setPosition(getPosition().plus(getSize().times(.25))).setWidth(getWidth() * .5).setHeight(getHeight() * .25).build()
        ...
        overlayRectangle = new BoxBuilder().setX(getX() + getWidth() * .25).setY(getY() + getHeight() * .5).setWidth(getWidth() * .5).setHeight(getHeight() * .25).build()
                                           .asScaledRectangle(scale);
