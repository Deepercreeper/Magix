...
public abstract class TestEntity extends Entity
{
    ...
    {
        super(new EntityBuilder().setX(x).setY(y).setWidth(width).setHeight(height).setMass(mass)
                                 .setElasticity(elasticity).setSpeed(speed));
    }

    @Override
    public Image generateImage(double scale)
    {
        // Image image = new Image.ImageBuilder().set(getDistanceBox(getVelocity(), 1. / 60).asScaledRectangle(scale)
        // ).build();
        // image.setData(Display.createRectangle(image.getWidth(), image.getHeight(), 0xff000000));
        ...
        Rectangle overlayRectangle = new BoxBuilder().setPosition(getPosition().plus(getSize().times(.25)))
                                                     .setSize(getSize().times(.5)).build().asScaledRectangle(scale);
        Image overlay = new Image.ImageBuilder().set(overlayRectangle).build();
        overlay.setData(Display
                .createFilledRectangle(overlay.getWidth(), overlay.getHeight(), 0xff000000 | color.getRGB()));
